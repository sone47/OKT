...
from .modules import KSE, OTE

class OKTNet(nn.Module):
  def __init__(self, n_skill, n_exercise, n_it, n_at, d_e, d_q, d_a, d_at, d_p, d_h, device=None, dropout=0.05):
    ...
    self.answer_embed = nn.Embedding(2, d_h)
  ...
  def forward(self, kc_data, a_data, e_data, it_data, at_data):
    ...
    for t in range(seq_len):
      ...
      # predict
      pred[:, t] = self.sig(self.predict(torch.cat((previous_h, q), 1)))

      # update
      h = self.kse(x, previous_h)
